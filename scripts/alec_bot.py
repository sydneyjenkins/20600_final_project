...
class AlecBot(object):
    ...
    def __init__(self, DEBUG=False):
        ...
        if self.DEBUG:
            rospy.init_node("alec_bot")

        # subscribe to the robot's RGB camera data stream
        self.image_sub = rospy.Subscriber('/alec_bot/camera/rgb/image_raw',
                Image, self.image_callback)

        # subscribe to the robot's scan topic
        rospy.Subscriber("/alec_bot/scan", LaserScan, self.process_scan)

        # set up publisher and Twist to publish to /cmd_vel
        self.cmd_vel_pub = rospy.Publisher('/alec_bot/cmd_vel', Twist, queue_size=10)
        self.twist = Twist()
